...
function showRAG()         { navigate('rag-strategies', 'toc-rag'); }
...
const PATTERNS = [
  {
    ...
    badge: { text: "Baseline", color: "#3b82f6" },
    desc: `The starting point for every RAG system.<br><br>
           <strong>Embed query → cosine similarity over all docs → top-K chunks → augment prompt → generate.</strong><br><br>
           <em>Weakness:</em> query phrasing mismatches go unhandled; no reranking.`,
    pipeline: [
      {t:"Embed Query",blue:true},{a:true},{t:"Vector Search",blue:true},{a:true},
      {t:"Augment Prompt",green:true},{a:true},{t:"Generate",amber:true},
    ],
    samples: ["What is the main topic of this document?","Summarise the key points","What conclusions are drawn?"],
    endpoint: "/rag/naive",
  },
  {
    id: "advanced", label: "Advanced RAG",
    badge: { text: "Production-Grade", color: "#a855f7" },
    desc: `Three upgrades over Naive RAG:<br><br>
           <strong>1. Query Rewriting</strong> — LLM rephrases for better retrieval.<br>
           <strong>2. Hybrid Search</strong> — dense + sparse in parallel.<br>
           <strong>3. Re-ranking</strong> — LLM scores candidates for precision.`,
    pipeline: [
      {t:"Rewrite",purple:true},{a:true},{t:"Hybrid Search",blue:true},{a:true},
      {t:"RRF Fusion",blue:true},{a:true},{t:"Re-rank",red:true},{a:true},{t:"Generate",amber:true},
    ],
    samples: ["Compare the different approaches mentioned","What are the technical details explained?","How does this relate to industry best practices?"],
    endpoint: "/rag/advanced",
  },
  {
    id: "agentic", label: "Agentic RAG",
    badge: { text: "Hot Topic", color: "#ef4444" },
    desc: `The LLM acts as an <strong>agent</strong> — it has a search tool and decides
           <em>if</em>, <em>when</em>, and <em>how many times</em> to retrieve.<br><br>
           Multi-hop questions become natural. It's a loop, not a pipeline.`,
    pipeline: [
      {t:"Agent Thinks",red:true},{a:true},{t:"search()",blue:true},{a:true},
      {t:"Agent Thinks",red:true},{a:true},{t:"repeat / stop",amber:true},{a:true},{t:"Answer",green:true},
    ],
    samples: ["What are all the risks mentioned and how are they addressed?","Find every recommendation made and explain why","What problem is being solved and what is the proposed solution?"],
    endpoint: "/rag/agentic",
  },
  {
    id: "hybrid", label: "Hybrid RAG",
    badge: { text: "Practical Best", color: "#22c55e" },
    desc: `<strong>Dense</strong> (semantic embeddings) + <strong>Sparse</strong> (BM25/TF-IDF)
           merged with Reciprocal Rank Fusion.<br><br>
           Dense handles paraphrase & synonyms. Sparse handles exact keywords.
           RRF merges both without a learned weight — simple and effective.`,
    pipeline: [
      {t:"Dense (Vector)",blue:true},{a:true},{t:"Sparse (BM25)",amber:true},{a:true},
      {t:"RRF Fusion",green:true},{a:true},{t:"Generate",amber:true},
    ],
    samples: ["What specific metrics or numbers are mentioned?","Explain the methodology used","What are the key definitions in this document?"],
    endpoint: "/rag/hybrid",
    showSources: true,
  },
  {
    id: "graph", label: "Graph RAG",
    badge: { text: "Complex Reasoning", color: "#f59e0b" },
    desc: `Documents are nodes in a <strong>knowledge graph</strong>. Retrieval seeds on the best
           match, then <strong>BFS-expands</strong> across edges to pull in related context.<br><br>
           Best for multi-hop and relational questions.`,
    pipeline: [
      {t:"Seed Search",blue:true},{a:true},{t:"Graph Hop 1",amber:true},{a:true},
      {t:"Graph Hop 2",amber:true},{a:true},{t:"Re-score",purple:true},{a:true},{t:"Generate",green:true},
    ],
    samples: ["How are the main concepts connected to each other?","What leads to the outcomes described?","Trace the relationship between the problem and the solution"],
    endpoint: "/rag/graph",
    showGraph: true,
  },
];
...
function pipelineHTML(steps) {
  return steps.map(s => {
    if (s.a) return `<span class="pipe-arrow">→</span>`;
    const cls = s.blue?"blue":s.green?"green":s.purple?"purple":s.amber?"amber":s.red?"red":"";
    return `<span class="pipe-box ${cls}">${s.t}</span>`;
  }).join("");
}

function initRAGStrategies() {
...
PATTERNS.forEach((p, idx) => {
  ...
  panel.innerHTML = `
    <div class="left-col">
      <div class="card">
        <span class="badge" style="background:${p.badge.color}18;color:${p.badge.color}">${p.badge.text}</span>
        <div class="desc">${p.desc}</div>
      </div>
      <div class="card">
        <div class="card-title">Pipeline</div>
        <div class="pipeline">${pipelineHTML(p.pipeline)}</div>
      </div>
    </div>
    ...
    <div class="right-col">
      <div class="card query-area">
        ...
        <textarea id="q-${p.id}" rows="3" placeholder="Upload a document, then ask a question…"></textarea>
        <div class="samples">
          ${p.samples.map(s=>`<button class="sample-btn" onclick="setSample('${p.id}','${s.replace(/'/g,"\\'")}')">${s}</button>`).join("")}
        </div>
        <button class="run-btn" id="btn-${p.id}"
          onclick="runQuery('${p.id}','${p.endpoint}',${!!p.showSources},${!!p.showGraph})">
          Run
        </button>
      </div>
      <div class="results-row">
        <div class="card">
          ...
          <div id="steps-${p.id}" class="steps-list">
            <div class="empty">Run a query to see the steps.</div>
          </div>
        </div>
        <div class="card">
          <div class="card-title">Retrieved Chunks</div>
          <div id="docs-${p.id}"><div class="empty">—</div></div>
        </div>
        <div class="card">
          <div class="card-title">Answer</div>
          <div id="answer-${p.id}" class="answer-box" style="color:var(--muted);font-size:13px">—</div>
        </div>
      </div>
    </div>
  `;
...
// Add Summary tab after RAG tabs
(function() {
  const btn = document.createElement("button");
  btn.className = "tab";
  btn.textContent = "Summary";
  btn.id = "tab-summary";
  btn.onclick = () => switchToSummary();
  tabBar.appendChild(btn);

  const panel = document.createElement("div");
  panel.id = "panel-summary";
  panel.className = "panel";
  panel.style.display = "none";
  panel.innerHTML = `
    <div style="grid-column:1/-1;overflow-y:auto;padding-right:4px">
      <div style="max-width:900px;margin:0 auto;display:flex;flex-direction:column;gap:20px">

        <div style="text-align:center;padding:8px 0 4px">
          <h2 style="font-size:22px;font-weight:800;letter-spacing:-0.4px">RAG Types — Simply Technical</h2>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
          <div class="card">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
              <span style="background:#3b82f622;color:#3b82f6;border:1px solid #3b82f6;border-radius:999px;font-size:11px;font-weight:700;padding:2px 10px">1</span>
              <span style="font-size:15px;font-weight:700">Naive RAG</span>
            </div>
            <p style="font-size:14px;color:var(--muted);line-height:1.75">
              Split documents into chunks → convert to vectors → store in vector DB → user asks a question →
              find closest matching chunks → send to LLM → get answer.<br><br>
              <strong style="color:var(--text)">That's it. No intelligence in between.</strong>
            </p>
          </div>

          <div class="card">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
              <span style="background:#a855f722;color:#a855f7;border:1px solid #a855f7;border-radius:999px;font-size:11px;font-weight:700;padding:2px 10px">2</span>
              <span style="font-size:15px;font-weight:700">Advanced RAG</span>
            </div>
            <p style="font-size:14px;color:var(--muted);line-height:1.75">
              Same as Naive, but smarter at every step:<br><br>
              <strong style="color:var(--text)">Before retrieval</strong> → clean up / rewrite the user's question first<br>
              <strong style="color:var(--text)">During retrieval</strong> → fetch better, more relevant chunks<br>
              <strong style="color:var(--text)">After retrieval</strong> → remove noise, re-rank results before sending to LLM<br><br>
              <em>Think of it as Naive RAG with a brain added at each stage.</em>
            </p>
          </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px">
          <div class="card">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
              <span style="background:#ef444422;color:#ef4444;border:1px solid #ef4444;border-radius:999px;font-size:11px;font-weight:700;padding:2px 10px">3</span>
              <span style="font-size:15px;font-weight:700">Agentic RAG</span>
            </div>
            <p style="font-size:14px;color:var(--muted);line-height:1.75">
              The LLM is not just answering — it's <strong style="color:var(--text)">deciding</strong>.
              It asks "do I need to search?", searches, reads the result, asks "is this enough?",
              searches again if needed, and only then answers.<br><br>
              Uses tools like web search, calculators, APIs.
              <strong style="color:var(--text)">Multi-step reasoning loop.</strong>
            </p>
          </div>

          <div class="card">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
              <span style="background:#22c55e22;color:#22c55e;border:1px solid #22c55e;border-radius:999px;font-size:11px;font-weight:700;padding:2px 10px">4</span>
              <span style="font-size:15px;font-weight:700">Hybrid RAG</span>
            </div>
            <p style="font-size:14px;color:var(--muted);line-height:1.75">
              Two searches run in parallel:<br><br>
              <strong style="color:#3b82f6">Dense search</strong> — finds chunks by meaning (semantic/embeddings)<br>
              <strong style="color:#f59e0b">Sparse search</strong> — finds chunks by exact keywords (BM25)<br><br>
              Both results are merged using <strong style="color:var(--text)">RRF (Reciprocal Rank Fusion)</strong>.
              You get the best of both searches in one result.
            </p>
          </div>

          <div class="card">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
              <span style="background:#f59e0b22;color:#f59e0b;border:1px solid #f59e0b;border-radius:999px;font-size:11px;font-weight:700;padding:2px 10px">5</span>
              <span style="font-size:15px;font-weight:700">Graph RAG</span>
            </div>
            <p style="font-size:14px;color:var(--muted);line-height:1.75">
              Instead of storing documents as chunks, you extract
              <strong style="color:var(--text)">entities and relationships</strong> and store them as a
              knowledge graph (nodes + edges). When a question comes in, you
              <strong style="color:var(--text)">traverse the graph</strong> to find connected facts —
              not just similar text. Great when answers depend on
              <em>how things relate</em>, not just what they say.
            </p>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:0;border:1px solid var(--border);border-radius:8px;overflow:hidden">
            <div style="background:#1e3a5f;padding:10px 14px;font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.06em">RAG Type</div>
            <div style="background:#1e3a5f;padding:10px 14px;font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.06em">Storage</div>
            <div style="background:#1e3a5f;padding:10px 14px;font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.06em">Notes</div>

            <div style="padding:10px 14px;border-top:1px solid var(--border);font-weight:600;font-size:13px">Naive &amp; Advanced</div>
            <div style="padding:10px 14px;border-top:1px solid var(--border);font-size:13px;color:#22c55e">Vector DB only ✅</div>
            <div style="padding:10px 14px;border-top:1px solid var(--border);font-size:12px;color:var(--muted)">Simplest setup</div>

            <div style="padding:10px 14px;border-top:1px solid var(--border);font-weight:600;font-size:13px">Hybrid</div>
            <div style="padding:10px 14px;border-top:1px solid var(--border);font-size:13px;color:#22c55e">Vector DB + BM25 ✅</div>
            <div style="padding:10px 14px;border-top:1px solid var(--border);font-size:12px;color:var(--muted)">Best retrieval recall in practice</div>

            <div style="padding:10px 14px;border-top:1px solid var(--border);font-weight:600;font-size:13px">Agentic</div>
            <div style="padding:10px 14px;border-top:1px solid var(--border);font-size:13px;color:#f59e0b">Vector DB optional ⚠️</div>
            <div style="padding:10px 14px;border-top:1px solid var(--border);font-size:12px;color:var(--muted)">Tool-agnostic — uses whatever retrieval makes sense</div>

            <div style="padding:10px 14px;border-top:1px solid var(--border);font-weight:600;font-size:13px">Graph RAG</div>
            <div style="padding:10px 14px;border-top:1px solid var(--border);font-size:13px;color:#ef4444">Graph DB, no Vector DB ❌</div>
            <div style="padding:10px 14px;border-top:1px solid var(--border);font-size:12px;color:var(--muted)">Neo4j etc. — replaces vector search entirely</div>
          </div>
        </div>
        ...
        <div class="card">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
            <span style="background:#6366f122;color:#6366f1;border:1px solid #6366f1;border-radius:999px;font-size:11px;font-weight:700;padding:2px 10px">Beyond Text</span>
            <span style="font-size:15px;font-weight:700">Multimodal RAG</span>
          </div>
          <p style="font-size:13px;color:var(--muted);line-height:1.85">
            In a multimodal RAG system, not only text but also <strong style="color:var(--text)">images, audio, and video</strong> can be converted into embeddings and stored as vectors in a vector database.
            Text uses <strong style="color:#3b82f6">text-embedding models</strong>, images use <strong style="color:#a855f7">vision-embedding models</strong>,
            audio uses <strong style="color:#22c55e">audio-embedding models</strong>, and video uses <strong style="color:#f59e0b">video-embedding models</strong> that capture both visual frames and sound.<br><br>
            All these modalities become numerical vectors that represent semantic meaning and can be stored in the same or separate indexes.
            During retrieval, a user query is embedded and matched against these vectors, allowing the system to return relevant text, images, audio, or video.<br><br>
            <em>The LLM never stores the raw content — it only receives whatever the vector DB retrieves at query time.</em>
          </p>
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:14px">
            <div style="background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:10px;text-align:center">
              ...
              <div style="font-size:11px;font-weight:700;color:#3b82f6">Text</div>
              <div style="font-size:10px;color:var(--muted);margin-top:2px">Text-embedding models</div>
            </div>
            <div style="background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:10px;text-align:center">
              <div style="font-size:18px;margin-bottom:4px">🖼️</div>
              <div style="font-size:11px;font-weight:700;color:#a855f7">Images</div>
              <div style="font-size:10px;color:var(--muted);margin-top:2px">Vision-embedding models</div>
            </div>
            <div style="background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:10px;text-align:center">
              <div style="font-size:18px;margin-bottom:4px">🎵</div>
              <div style="font-size:11px;font-weight:700;color:#22c55e">Audio</div>
              <div style="font-size:10px;color:var(--muted);margin-top:2px">Audio-embedding models</div>
            </div>
            <div style="background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:10px;text-align:center">
              <div style="font-size:18px;margin-bottom:4px">🎬</div>
              <div style="font-size:11px;font-weight:700;color:#f59e0b">Video</div>
              <div style="font-size:10px;color:var(--muted);margin-top:2px">Video-embedding models</div>
            </div>
          </div>
        </div>

        <!-- Key takeaway -->
        <div style="background:#0d2240;border:1px solid var(--blue);border-radius:var(--radius);padding:16px 20px">
          <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);margin-bottom:8px">Key Takeaway</div>
          <p style="font-size:14px;line-height:1.8;color:var(--text)">
            "Vector DB is the backbone of most RAG systems, but
            <strong style="color:#f59e0b">Agentic RAG is tool-agnostic</strong> and
            <strong style="color:#ef4444">Graph RAG replaces it entirely</strong> with a graph database."
          </p>
        </div>
...
} // end initRAGStrategies

function switchToSummary() {
  document.querySelectorAll(".tab").forEach(t => t.classList.remove("active"));
  document.getElementById("tab-summary").classList.add("active");
  PATTERNS.forEach(p => document.getElementById(`panel-${p.id}`).classList.remove("active"));
  const sp = document.getElementById("panel-summary");
...
function switchTab(idx) {
  ...
  PATTERNS.forEach((p,i) => document.getElementById(`panel-${p.id}`).classList.toggle("active", i===idx));
  // hide summary panel
  const sp = document.getElementById("panel-summary");
...
function setSample(id, text) {
  document.getElementById(`q-${id}`).value = text;
}
...
async function uploadFile(input) {
  ...
  try {
    ...
    if (data.error || !data.chunks) {
      ...
      status.textContent = `Error: ${data.error || "Upload failed — check server logs"}`;
    } else {
      status.className = "upload-status ok";
      status.textContent = `✓ ${data.filename || file.name} — ${data.chunks} chunks indexed`;
    }
  } catch (e) {
    status.className = "upload-status err";
...
function clearAll() {
  PATTERNS.forEach(p => {
    document.getElementById(`q-${p.id}`).value = "";
    document.getElementById(`steps-${p.id}`).innerHTML = `<div class="empty">Run a query to see the steps.</div>`;
    document.getElementById(`docs-${p.id}`).innerHTML  = `<div class="empty">—</div>`;
    document.getElementById(`answer-${p.id}`).textContent = "—";
  });
}
