...
def lc_rag(req: QuestionRequest):
    ...
    try:
        ...
        answer      = (ChatPromptTemplate.from_template(
            ...
            | chat_model | StrOutputParser()).invoke({"ctx": context, "q": req.question})
    ...
    try:
        ...
        if tool_calls:
            ...
            answer = (ChatPromptTemplate.from_template(
                ...
                | chat_model | StrOutputParser()).invoke({"q": req.question, "ctx": ctx})
        else:
...
def lc_parsers(req: TopicRequest):
    ...
    try:
        ...
        list_output = (ChatPromptTemplate.from_template(
            "List five items related to '{t}'. {fi}").partial(fi=list_parser.get_format_instructions())
            | chat_model | list_parser).invoke({"t": req.topic})

        json_output = (ChatPromptTemplate.from_template(
            "Return JSON about '{t}' with keys: name, description, key_facts (array of 3). {fi}")
            .partial(fi=json_parser.get_format_instructions())
            | chat_model | json_parser).invoke({"t": req.topic})

        str_output = (ChatPromptTemplate.from_template(
            "Write one sentence explaining '{t}'.") | chat_model | StrOutputParser()
        ).invoke({"t": req.topic})
...
def lc_agent_ep(req: QuestionRequest):
    ...
    try:
        ...
        agent      = create_react_agent(
            chat_model, [calculator, get_exchange_rate, get_country_info],
            ...
            if isinstance(msg, AIMessage) and msg.tool_calls:
                for tool_call in msg.tool_calls:
                    pending[tool_call["id"]] = {
                        "tool": tool_call["name"], "input": tool_call["args"], "result": ""
                    }
    ...
    try:
        ...
        def research_node(state: BlogState) -> BlogState:
            output = (ChatPromptTemplate.from_template(
                "Research '{t}'. Return 5 concise bullet-point facts.") | chat_model | str_parser
